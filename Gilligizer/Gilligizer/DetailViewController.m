...
- (void)configureView
{
    ...
    if (self.detailItem) {
        ...
        self.episodeIDField.text = [NSString stringWithFormat:@"%ld", [[self.detailItem valueForKey:@"episodeID"] integerValue]];
...
{
    [self.detailItem setValue:self.descriptionView.text forKey:@"desc"];
}
